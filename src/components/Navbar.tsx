...
import { openServiceDetail, openIndustryDetail } from '../lib/eventBus';
...
export default function Navbar() {
  ...
  const path = usePath();
  const isCareersPage = path.replace(/\/+$/, '') === '/careers';
  ...
  // Open a specific Service/Industry detail modal directly from the mega menu,
  // routing home first (and giving the section time to mount) if we're on another page.
  ...
  const openIndustry = (id: string) => {
    ...
    if (window.location.pathname !== '/') {
      navigate('/');
      setTimeout(() => {
        scrollToTarget('#industries');
        openIndustryDetail(id);
      }, 80);
    } else {
      scrollToTarget('#industries');
      openIndustryDetail(id);
  ...
  return (
    <>
      ...
      <nav
        id="navbar-sticky"
        className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 bg-slate-900 border-b border-slate-800 ${
          scrolled ? 'backdrop-blur-md shadow-md py-3.5' : 'py-5'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            >
              <button
                className={`text-sm font-medium text-slate-300 hover:text-brand-primary flex items-center gap-1 py-2 transition-colors ${
                  activeMegaMenu === 'about' ? 'text-brand-primary' : ''
                }`}
              >
                About
            ...
            >
              <button
                className={`text-sm font-medium text-slate-300 hover:text-brand-primary flex items-center gap-1 py-2 transition-colors ${
                  activeMegaMenu === 'services' ? 'text-brand-primary' : ''
                }`}
              >
                Services
            ...
            >
              <button
                className={`text-sm font-medium text-slate-300 hover:text-brand-primary flex items-center gap-1 py-2 transition-colors ${
                  activeMegaMenu === 'industries' ? 'text-brand-primary' : ''
                }`}
              >
                Industry
            ...
            <button
              onClick={goToCareers}
              className={`text-sm font-medium hover:text-brand-dark dark:hover:text-brand-primary transition-colors ${
                isCareersPage ? 'text-brand-primary' : 'text-slate-300'
              }`}
            >
              Careers
            </button>
          ...
          <div className="hidden lg:flex items-center gap-4">
            <a
              ...
              data-cursor="hover"
              className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-white bg-brand-primary hover:bg-brand-dark transition-colors py-2 px-3.5 rounded-full shadow-md shadow-brand-primary/20"
            >
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full bg-slate-800 text-slate-300 hover:bg-slate-700 transition-colors cursor-pointer flex items-center justify-center border border-slate-700"
          ...
          <div className="lg:hidden flex items-center gap-3">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-lg bg-slate-800 text-slate-300 hover:bg-slate-700 transition-colors cursor-pointer border border-slate-700"
              aria-label="Toggle theme"
            >
              {theme === 'light' ? <Moon className="w-4.5 h-4.5" /> : <Sun className="w-4.5 h-4.5 text-brand-gold" />}
            </button>
            <a href="tel:+919687412080" className="p-2 text-slate-400 hover:text-brand-primary transition-colors">
              <Phone className="w-4 h-4" />
            </a>
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="p-2 rounded-lg hover:bg-slate-800 text-slate-200 transition-colors cursor-pointer"
            ...
            >
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-4 px-6">
                  ...
                  >
                    <span>Careers</span>
                    <span className="text-[10px] bg-emerald-500/15 text-brand-primary px-2 py-0.5 rounded-full font-bold">We're hiring</span>
                  </button>
                </div>
              </div>
